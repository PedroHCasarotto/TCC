...
def out_decode(model, data, device, args):  
    ...
    if args.gene_encoder == "tenfold_binary":
        x=torch.FloatTensor(tenfold_binary(data.x, decimal_points=args.decimal_points, max_int=args.max_int)).to(device)
    elif args.gene_encoder == "binary":
        x=torch.FloatTensor(binary(data.x, decimal_points=args.decimal_points, max_int=args.max_int)).to(device)
    elif args.gene_encoder == "one_hot":
        x=torch.FloatTensor(one_hot(data.x, decimal_points=args.decimal_points, max_int=args.max_int)).to(device)
    elif args.gene_encoder == "value":
        x=torch.FloatTensor(value(data.x, decimal_points=args.decimal_points, max_int=args.max_int)).to(device)
    x=torch.cat((x,data.cell_type.repeat(args.gene_e_length,1)),1)
    x = model.gene_embedding(x)
    x = model.position(x)
    softmax=nn.Softmax(dim=1)
    # for _ in range(args.max_len):
    for _ in tqdm(range(args.max_len), desc="Generating sequences"):
        all_seq=[]
        for i in y_pred:
            if _==0:
...
out_path = 'out'
# model_weights_path = out_path+"/ckpts_{}/{}/dim{}_n6h8ff512_bh4_lr0.0001/ckpt0".format(args.dataset,args.gene_encoder, args.hidden_dim)
# model_weights_path = out_path+'/ckpts{}_encoder{}_decimal{}_maxint{}_genelength{}_dim{}_n{}h{}ff{}_bh{}_lr{}/ckpt0'.format(args.dataset,args.gene_encoder,
#             args.decimal_points, args.max_int, args.gene_e_length, args.hidden_dim,args.TF_N,args.TF_H,args.ff_dim,args.batch_size,args.lr)
model_weights_path = args.out_path+"ckpts_{}/decimal_points_{}/{}/maxint{}_genelength{}_dim{}_n{}h{}ff{}_bh{}_lr{}/best_weights".format(
            args.dataset,args.decimal_points,args.gene_encoder, 
            args.max_int, args.gene_e_length, args.hidden_dim,
            args.TF_N, args.TF_H, args.ff_dim,
            args.batch_size, args.lr)

ckpt = torch.load(model_weights_path, map_location=torch.device('cpu'))
...
for i in range(len(test_dataset)):
    ...
    try:
        ...
        smallest_10 = heapq.nsmallest(10, SA_scores) 
        aa.append(np.median(smallest_10))
        aa.append(str(smallest_10)) 
    except:
        aa.append('Returned Error')
        aa.append('Returned Error')      
    OUT.append(aa)
    
# Save test results
floder="./result/{}_test/decimal_points_{}/{}".format(
    args.dataset,args.decimal_points,args.gene_encoder)
...
df.to_csv("./result/{}_test/decimal_points_{}/{}/_maxint_{}_genelength{}_dim{}_n{}h{}ff{}_bh{}_lr{}.csv".format(
            args.dataset,args.decimal_points,args.gene_encoder, 
            args.max_int, args.gene_e_length, args.hidden_dim,
            args.TF_N, args.TF_H, args.ff_dim,
            args.batch_size, args.lr), index=False)
df.head()
