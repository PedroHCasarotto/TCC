...
from .gene_ex_encoder_upgraded import *
...
class TransGTM(nn.Module):
    def __init__(self, mode, args):
        ...
        self.h=args.TF_H
        self.decimal_points = args.decimal_points
        self.max_int = args.max_int
        self.gene_e_length = args.gene_e_length
        self.tgt_embedding = Embeddings(args.hidden_dim, args.vocab_size)
        if args.gene_encoder == "tenfold_binary":
            self.gene_embedding = nn.Linear(args.cell_types_length +
            (1 + (10**(args.decimal_points+1)).bit_length() if args.max_int != 0 
            else 1 + (10**(args.decimal_points)).bit_length()), args.hidden_dim)
            
        elif args.gene_encoder == "binary":
            self.gene_embedding = nn.Linear(args.cell_types_length + ( 1 + (args.max_int).bit_length() + 4*args.decimal_points), args.hidden_dim)
        elif args.gene_encoder == "one_hot":                                            # to add 0
            self.gene_embedding = nn.Linear(args.cell_types_length + ( 1 + (args.max_int+1) + 9*args.decimal_points), args.hidden_dim) 
        elif args.gene_encoder == "value":
            self.gene_embedding = nn.Linear(args.cell_types_length + (1), args.hidden_dim) # 3
        else:
    ...
    def forward(self, data, device):
        ...
        for i in range(len(data.ptr)-1):
            if self.param.gene_encoder == "tenfold_binary":
                x=torch.FloatTensor(tenfold_binary(data[i].x, decimal_points=self.decimal_points, max_int=self.max_int)).to(device)
            elif self.param.gene_encoder == "binary":
                x=torch.FloatTensor(binary(data[i].x, decimal_points=self.decimal_points, max_int=self.max_int)).to(device)
            elif self.param.gene_encoder == "one_hot":
                x=torch.FloatTensor(one_hot(data[i].x, decimal_points=self.decimal_points, max_int=self.max_int)).to(device)
            elif self.param.gene_encoder == "value":
                x=torch.FloatTensor(value(data[i].x, decimal_points=self.decimal_points)).to(device)
            else:
                print("Please imput any one of [tenfold_binary, binary, one_hot, value]")
            x=torch.cat((x,data[i].cell_type.repeat(self.gene_e_length,1)),1)
