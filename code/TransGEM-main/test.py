...
def out_decode(model, data, device, args):  
    ...
    if args.gene_encoder == "tenfold_binary":
        x=torch.FloatTensor(tenfold_binary(data.x, decimal_points=args.decimal_points, decimals_to_int=args.decimal_points)).to(device)
    elif args.gene_encoder == "binary":
        x=torch.FloatTensor(binary(data.x, decimal_points=args.decimal_points)).to(device)
    elif args.gene_encoder == "one_hot":
        x=torch.FloatTensor(one_hot(data.x, decimal_points=args.decimal_points)).to(device)
    elif args.gene_encoder == "value":
        x=torch.FloatTensor(value(data.x, decimal_points=args.decimal_points)).to(device)
...
for i in range(2):
    ...
    aa.append('See after') # aa.append(np.median(heapq.nsmallest(10, SA_scores)))
    aa.append('See after') # aa.append(str(heapq.nsmallest(10, SA_scores)))
    OUT.append(aa)
