...
class TransGTM(nn.Module):
    def __init__(self, mode, args):
        ...
        if args.gene_encoder == "tenfold_binary":
            self.gene_embedding = nn.Linear(15 + 1+len(bin(15*(10**args.decimals_to_int))[2:]),args.hidden_dim) # 25
        elif args.gene_encoder == "binary":
            self.gene_embedding = nn.Linear(15 + 1+5+4*args.decimal_points,args.hidden_dim) # 30
        elif args.gene_encoder == "one_hot":
            self.gene_embedding = nn.Linear(15 + 1+15+9*args.decimal_points+1,args.hidden_dim) # 71
        elif args.gene_encoder == "value":
            self.gene_embedding = nn.Linear(15 + 1,args.hidden_dim) # 1
        else:
    ...
    def forward(self, data, device):
        ...
        for i in range(len(data.ptr)-1):
            if self.param.gene_encoder == "tenfold_binary":
                x=torch.FloatTensor(tenfold_binary(data[i].x, decimal_points=self.decimal_points, decimals_to_int=self.decimal_points)).to(device)
            elif self.param.gene_encoder == "binary":
                x=torch.FloatTensor(binary(data[i].x, decimal_points=self.decimal_points)).to(device)
            elif self.param.gene_encoder == "one_hot":
                x=torch.FloatTensor(one_hot(data[i].x, decimal_points=self.decimal_points)).to(device)
