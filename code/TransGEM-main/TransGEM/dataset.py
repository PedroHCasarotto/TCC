...
def load_dataset(path, dataset, train_path, val_path, teat_path):
    ...
    selfies_train,gene_e_train=list(df_train["selfies"]), list(df_train["gene_e"])
    selfies_val, gene_e_val = list(df_val["selfies"]), list(df_val["gene_e"])
    selfies_test, gene_e_test = list(df_test["selfies"]), list(df_test["gene_e"])
    ...
    mydataset = MyDataset(path, dataset, target, gene_List=gene_List)
...
def bulit_test_dataset(path, dataset, ge_list, cell_line=None, sf_list=None):
    ...
    mydataset = MyDataset(path, dataset, target, gene_List)
...
class MyDataset(InMemoryDataset):
    ...
    def __init__(self, path, dataset, target, gene_List, transform=None, pre_transform=None, pre_filter=None):
